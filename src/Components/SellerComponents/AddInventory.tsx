...
const InventoryList = () => {
  ...
  const handleAddItem = () => {
    ...
    if(Number(quantity)<quantityduplicate)
      {
        ToastAndroid.show(`Add more quantity not less than   Quantity`,ToastAndroid.SHORT);
        return
      }
    if (quantity) {
      if (type === 'sizeColorQuantity') {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        ...
        if (id) {
          ...
          if(Number(lowLevelThreshold)> (totalQuantity))
            {
              ToastAndroid.show("Low level threshold must be lesser than total Quantity",ToastAndroid.SHORT)
              return
            }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        quantityduplicate=data?.totalQuantity
