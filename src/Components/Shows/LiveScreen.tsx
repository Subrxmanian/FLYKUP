...
import Icon from 'react-native-vector-icons/MaterialIcons'; // 3-Dot Icon
...
const LiveStreamScreen = () => {
  ...
  const renderProduct = ({item}) => {
    ...
    const handlePayment = async (numericAmount:any) => {
      try {
        ...
        // navigation.navigate("PaymentSuccess", { numericAmount,product:item })
        // return
    ...
    return (
      ...
      <View style={styles.productBox}>
        ...
        <View style={{width:'50%'}}>
          ...
          <Text style={{color: 'white',width:'100%'}}>{item.description}</Text>
          <View style={{flexDirection:'row',gap:5}}>
            {/* <mater */}
            <MaterialCommunityIcons name='cart-variant' color='#fff' size={20} />
          <Text style={{color: 'white',width:'100%'}}>{item.quantity}</Text>
          </View>
        </View>
      ...
      <>
       <View style={{flexDirection:'row',alignItems:'center',marginTop:10,alignSelf:'center'}}>
        ...
        <Text style={{color:'white',fontSize:18}}> Starting Price: </Text>
        <MaterialIcons name='currency-rupee' color='white' size={20} />
        <Text style={{color:'white',fontSize:18}}>{item.startingPrice}</Text>
      </View>
      <View style={{flexDirection:'row',marginTop:10,alignSelf:'center',justifyContent:'space-between'}}>
        ...
        <Text style={{color:'white',fontSize:18}}> Reserved Price: </Text>
        <MaterialIcons name='currency-rupee' color='white' size={20} />
        <Text style={{color:'white',fontSize:18}}>{item.reservedPrice}</Text>
      </View>
      </>
       :null}
      {selectedTab === 'Buy Now'? <View style={{flexDirection:'row',alignItems:'center',marginTop:10,alignSelf:'center'}}>
        <AntDesign name='tags' color='#ffbe00' size={20}/>
        <Text style={{color:'white',fontSize:18}}> Product Price: </Text>
        <MaterialIcons name='currency-rupee' color='white' size={20} />
        <Text style={{color:'white',fontSize:18}}>{item.productPrice}</Text>
      </View>
       :null}
      {selectedTab === 'Buy Now'?  <TouchableOpacity style={styles.buyButton} onPress={()=>handlePayment(item.productPrice)}>
          <Text style={{fontSize:16,fontWeight:'600'}}>₹ Buy Now</Text>
        </TouchableOpacity>
       :null}
        {selectedTab === 'Give away'?  
        <TouchableOpacity style={[styles.buyButton,{backgroundColor:'green'}]}>
          <AntDesign name='gift' size={25} color='#fff'/>
          <Text style={{fontSize:16,fontWeight:'600',color:'white'}}> Get Now</Text>
        </TouchableOpacity>
       :null}
  ...
  useEffect(() => {
    const fetchUser = async () => {
        try {
          const response = await api.get(`/live/shows/get/${stream?.stream?._id}`);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.streamerInfo}>
          <TouchableOpacity
            ...
            <View style={[styles.streamerImage,{borderRadius:50,backgroundColor:'#455a64'}]}>
            <Text style={{fontSize:18,color:'white',fontWeight:'600'}}>{(sellerData?.basicInfo?.name)?.charAt(0)}</Text></View>}
           
          </TouchableOpacity>
          <View style={styles.streamerDetails}>
            <Text style={styles.streamerName}>{sellerData?.basicInfo?.name}</Text>
